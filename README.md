...
When VS Code opens, choose the model from the extension or UI you normally use and send exactly: `read AGENTS.md and follow it`. The generated `AGENTS.md` tells the model to read `MODEL_VALIDATOR_TASK.md`, edit only the prepared workspace, run every required public self-check command after editing and report each command with its exit code. Return to the terminal and press Enter only after the model reports those self-check results.

The CLI then asks which model was actually used. For example, if Copilot Chat reports `Raptor mini`, record the model as `raptor mini` and leave version blank if the UI does not show one. The values are stored in the generated target JSON and the attempt's `adapter-output/interactive-metadata.json`.
